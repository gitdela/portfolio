...
/** An aside inside an article body. */
function callout(tone: "note" | "tip" | "warning", content: string): Record<string, unknown> {
  return { _type: "callout", _key: nextKey(), tone, content };
}
...
const posts: SanityDocument[] = [
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    body: [
      ...
      callout(
        "warning",
        "If you take one thing from this: never let “the lookup failed” and “the thing is not there” collapse into the same branch in code that touches money.",
      ),
  ...
  },
  {
    ...
    body: [
      ...
      callout(
        "note",
        "This has an unglamorous cost. The shared development environment file is rejected by three of the roles, so those three carry their own derived files, and when the shared one changes they have to be regenerated. I have not found a version of this that is both strict and convenient.",
      ),
